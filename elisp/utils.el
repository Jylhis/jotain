...
(defun jotain-utils-find-org-files-recursively (directory)
  ...
  (when (and directory (file-exists-p directory) (file-directory-p directory))
    (let ((files '()))
      ;; PERFORMANCE: Set INCLUDE-DIRECTORIES to nil in directory-files-recursively
      ;; natively filters out directories, eliminating the need for redundant
      ;; file-regular-p checks and reducing I/O stat overhead in the search loop.
