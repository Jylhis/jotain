...
;;; @doc files in any modern repo (CI, k8s, helm).
(use-package yaml-mode
  :defer t)

;;; @doc CSV major mode with column alignment. csv-align-mode renders
;;; @doc separators visually so wide files become readable without
;;; @doc reflowing the actual bytes.
...
;;; @doc and CTEs aligned without manual whitespace fiddling.
(use-package sql-indent
  :defer t)

;;; @doc Jinja2 / Ansible / Saltstack templating syntax. Mode regex
;;; @doc covers `.j2`, `.jinja`, and `.jinja2`.
(use-package jinja2-mode
  :mode (("\\.j2\\'"      . jinja2-mode)
         ("\\.jinja2?\\'" . jinja2-mode)))

;;; @doc Major mode for gnuplot script files (`.plt`). Useful when an
;;; @doc analysis pipeline emits its own plotting scripts.
(use-package gnuplot
