{
  ...
  outputs = inputs @ { self, nixpkgs, emacs-overlay, darwin-emacs, flake-parts, systems, treefmt-nix, home-manager, ... }:
    flake-parts.lib.mkFlake { inherit inputs; } {
      ...
      perSystem = { config, self', inputs', pkgs, system, lib, ... }:
        let
          ...
          emacsOverlays =
            if isDarwin then [
              # On darwin: use nix-darwin-emacs for Emacs builds,
              # emacs-overlay for MELPA/ELPA package sets
              darwin-emacs.overlays.emacs
              emacs-overlay.overlays.package
            ] else [
              # On Linux: emacs-overlay provides both builds and packages
