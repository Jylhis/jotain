...
<Note>
The dev-shell `just` launch recipes (`run`, `debug`, `tty`, `daemon`,
`client`, `client-tty`, `quick`) are **temporarily disabled stubs**
because Emacs is not currently in the devenv shell (see the top-of-file
note in `devenv.nix`). For now, launch with `just run-built` (builds
Emacs via Nix, then launches it). The Home Manager daemon and the
`jotain-editor` / `jotain-visual` wrappers and `emd` / `em` / `emg`
aliases described below are **unaffected** — they run the Nix-built
Emacs directly, not the dev-shell binary.
</Note>
