...
to `command-completion-default-include-p`. Commands a package has tagged with
`(interactive nil foo-mode)` only appear when `foo-mode` is active; untagged commands are always
shown, and typing a full command name still runs it whether or not the filter would hide it. Two
stricter predicates ship with Emacs if you want fewer candidates: `command-completion-using-modes-and-keymaps-p`
(only commands tied to active modes or active keybindings) and `command-completion-using-modes-p`
(only commands explicitly tagged for a mode).
