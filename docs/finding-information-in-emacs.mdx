...
Jotain ships its own Info manual (`(jotain)`) alongside the bundled Emacs manuals. `C-h i d m
Jotain RET` opens it; it covers installation, the module layout, the Nix build layer, and the
full Nix module option reference. `M-x consult-info` includes it in cross-manual search. For
NixOS / nix-darwin / Home Manager users the manual is pre-wired via `INFOPATH`; for source
checkouts, run `just info` to build `result-info/` and `init-docs.el` picks it up automatically.
...
Jotain narrows the `M-x` candidate list via `read-extended-command-predicate`, set in `init-core`
to `command-completion-default-include-p`. A command opts in by listing applicable modes after the
`interactive` argument descriptor — `(interactive nil foo-mode)` for a no-argument command,
`(interactive "p" foo-mode bar-mode)` for one that reads a prefix argument and applies in either
mode — and then only appears in `M-x` when one of those modes is active. Untagged commands are
always shown, and typing a full command name still runs it whether or not the filter would hide
it. Two stricter predicates ship with Emacs if you want fewer candidates:
`command-completion-using-modes-and-keymaps-p` (only commands tied to active modes or active
keybindings) and `command-completion-using-modes-p` (only commands explicitly tagged for a mode).
