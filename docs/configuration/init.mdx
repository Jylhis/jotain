...
4. `require` each module in the right order. Archive refresh is *not* on the startup path: a background refresh runs on an idle timer only when the newest cached `archive-contents` is older than 7 days (or an archive has no cache at all); set `JOTAIN_NO_PACKAGE_REFRESH` to opt out entirely.
