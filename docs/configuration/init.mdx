...
A Nix-built Emacs (`just build`, the flake `packages.<system>.default`) provides most packages via `load-path`, so `use-package` finds them without touching the network. The archives above only come into play for packages that Nix does not ship.
