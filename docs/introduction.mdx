...
- A Nix expression (`emacs.nix`) for building GNU Emacs from source, with every upstream build flag exposed and `pkgs.emacs-git` (from [`nix-community/emacs-overlay`](https://github.com/nix-community/emacs-overlay)) binary-cache parity as an invariant
...
- **Nix-based builds** — reproducible Emacs from source with precise control over compile options, and binary-cache parity with `pkgs.emacs-git` (emacs-overlay) for the default configuration.
