...
The `basePackage.override` call is split into `baseOverrideArgs` (flags present in `make-emacs.nix` since nixpkgs 24.05) and `modernOverrideArgs` (flags added later, ≈24.11). The modern set is merged in only when `lib.versionAtLeast lib.version "24.11"`, so the build still evaluates when a downstream flake overrides `nixpkgs` with an older release. On the pinned unstable the gate is always true, so the full arg set is forwarded and cache parity is unaffected.
