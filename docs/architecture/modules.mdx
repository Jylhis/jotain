...
4. Run `just check` — it runs `nix flake check`, which both checks balanced parens (`elisp-lint`) and byte-compiles with warnings-as-errors (`elisp-compile`). (`just compile` is a disabled stub while Emacs is out of the dev shell; `just check` covers the same byte-compile warnings.)
