...
- **`init-lang-systems`** — C/C++ (`c-ts-mode`/`c++-ts-mode`, routed via `jotain-prog-ts-remaps`; eglot wires `clangd`, apheleia wires `clang-format`, dape uses `codelldb`), CUDA (`.cu`/`.cuh` → `c++-ts-mode`, since CUDA has no tree-sitter mode of its own — the cpp grammar and the full C++ tool stack apply), CMake, Meson, Haskell, OCaml (`tuareg` + `dune`, eglot wires `ocamllsp`), Zig (`zig-ts-mode`, eglot wires `zls`). `cc-mode` is retained for its `auto-mode-alist` decisions and as the grammarless fallback.
