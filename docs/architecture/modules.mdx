...
- **`init-lang-go`** — `go-ts-mode` / `go-mod-ts-mode` (built-in), eglot wires `gopls` in `init-prog` with a workspace config that turns on inlay hints, staticcheck, and the `unusedparams`/`shadow`/`nilness` analyses. Format-on-save runs gofmt through apheleia. Adds `go-tag` (struct-tag editing) and `gotest` (point-aware test runs) under the buffer-local `C-c C-t` prefix. gopls / dlv / gotests / gomodifytags come from the project environment, not from this config.
