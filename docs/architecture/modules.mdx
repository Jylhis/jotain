...
- **`init-lang-web`** — `typescript-ts-mode`, `tsx-ts-mode`, CSS, JSON, and related tree-sitter modes. Plain `.html`/`.htm` uses the built-in `mhtml-ts-mode` (tree-sitter HTML + JS + CSS); `web-mode` is kept for the templating dialects it uniquely handles (`.php`, `.erb`, `.jsp`, `.tpl`, mustache, Django).
- **`init-lang-devops`** — Dockerfile, Terraform (`*.tf`), gitlab-ci, justfile, Ansible, Bazel/Starlark (`BUILD`, `WORKSPACE`, `MODULE.bazel`, `*.bzl`, `.bazelrc`, …). A `jotain-docker-backend` defcustom (`'podman` by default, set to `'docker` for classic Docker) switches the runtime command used by `dockerfile-mode`. Bazel buffers format on save via buildifier (centralised through apheleia in `init-prog`).
- **`init-lang-data`** — YAML, CSV (with `csv-align-mode`), SQL (`sql-indent`), Jinja2, gnuplot.
- **`init-lang-systems`** — C/C++ (`c-ts-mode`/`c++-ts-mode`, routed via `jotain-prog-ts-remaps`; eglot wires `clangd`, apheleia wires `clang-format`, dape uses `codelldb`), CUDA (`.cu`/`.cuh` → `cuda-ts-mode`, defined here on the `cuda` tree-sitter grammar — a C++ superset; via `derived-mode-add-parents` it counts as `c++-ts-mode`, so the full C++ tool stack applies), CMake, Meson, Haskell (`haskell-ts-mode`, derived from `haskell-mode` so eglot's HLS entry matches), OCaml (`neocaml` — `neocaml-mode`/`neocaml-interface-mode` on the `ocaml`/`ocaml-interface` grammars; `tuareg` kept for `.mll`/`.mly`; eglot wires `ocamllsp`, apheleia wires `ocamlformat`), Zig (`zig-ts-mode`, eglot wires `zls`). `cc-mode` is retained for its `auto-mode-alist` decisions and as the grammarless fallback.
