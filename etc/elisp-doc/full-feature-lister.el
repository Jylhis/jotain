;;; full-feature-lister.el --- Parse load-path and list all features available for requiring.  -*- lexical-binding: t; -*-
...
(defun elisp-doc--docgen//update-hash ()
  ...
  (let ((el-file-regexp "^\\([^=].*\\)\\.el\\(\\.\\(gz\\|Z\\)\\)?$")
        (load-path (cl-remove-if (lambda (p) (string-match "lisp/obsolete$" p))
                                 load-path))
  ...
  (setq package--builtins
        (sort package--builtins
              (lambda (a b) (string< (symbol-name (car a))
                                     (symbol-name (car b)))))))
;;;###autoload
(defun elisp-doc--docgen//full-feature-lister (&optional rescan include-loaded sort)
...
`load-path' with those already loaded (listed in `features')."
  (interactive "P")
  (when rescan (elisp-doc--docgen//update-hash))
  (let ((out (if include-loaded features nil)))
    (maphash (lambda (_k v) (setq out (append out v)))
             finder-keywords-hash)
