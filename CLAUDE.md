...
- `just build-staged` — EXPERIMENTAL two-stage from-source Emacs build (`nix/emacs-staged.nix`): the C half (temacs) and the Lisp half as separate derivations, so Lisp-side patches never re-pay the C compile. Terminal-only, non-native-comp, not cache-parity, never CI.
- `just elc-parity` — diagnostic byte-diff of the monolithic vs per-file config compiles (`nix/elc-parity.nix`). A DIFFERS line is a bug report against `lisp/` (usually a missing `defvar`/`declare-function` stub), deliberately not a flake check.
