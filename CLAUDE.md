...
- **Full validation** (`.github/workflows/deploy.yml`, pushes to `main`/`next` plus manual dispatch) runs the complete `nix flake check` — including the heavy package builds and `elisp-test` that PR CI skips — plus `devenv test`, and pushes newly built artifacts to the `jylhis` cachix cache. On `main` it additionally publishes to **GitHub Pages**: `build-pages` builds the full `.#site` (including the `/help/api/` reference) and `deploy-pages` publishes it, served as this repo's project site at **page.jylhis.com/jotain/** (the site is built with `nix/site.nix` `baseHref = "/jotain"` to match). Its concurrency group never cancels in-progress runs, so post-merge cache pushes always finish.
...
- `just site` / `just serve-site` — build the full page.jylhis.com/jotain site (`.#site` → `result-site/`) / serve it locally (mounted under `/jotain/` to match the deploy base path).
...
- `packages.<system>.site` — the full page.jylhis.com/jotain site assembly from `nix/site.nix` (website shell + rendered docs + manuals + man page + the `/help/api/` reference), built under `baseHref = "/jotain"`; deployed from `main` to GitHub Pages (`build-pages`/`deploy-pages`)
