...
**Older-nixpkgs portability.** The `basePackage.override` call is split into `baseOverrideArgs` (flags present since nixpkgs 24.05) and `modernOverrideArgs` (flags `make-emacs.nix` grew later, ≈24.11: `noGui`, `srcRepo`, `withXinput2`, `withAlsaLib`, `withAcl`, `withGcMarkTrace`, `withGlibNetworking`). The modern set is merged in only when `lib.versionAtLeast lib.version "24.11"`, so a downstream flake that overrides `nixpkgs` with an older release (e.g. via `inputs.jotain.inputs.nixpkgs.follows`) still **evaluates and builds** — older `make-emacs.nix` would otherwise throw "called with unexpected argument". On the pinned unstable the gate is always true, so the full arg set is passed and cache parity is unchanged. Caveat: on 24.05 `pkgs.emacs` is Emacs 29, but the Elisp config targets Emacs 30/31 — the Nix layer builds, full runtime correctness on 29 is not guaranteed.

**`overlay.nix`** is a nixpkgs overlay providing `jotainEmacs` (bare Emacs from `emacs.nix`), `jylhisEmacs` (bare Emacs from the pinned `github:jylhis/emacs` Meson fork), `jotainInfo` (the `jotain.info` manual generated by `nix/info-manual.nix`), `jotainEmacsPackages` (full distribution — use-package auto-mapping + tree-sitter grammars + a makeBinaryWrapper that prepends `${jotainInfo}/share/info` to `INFOPATH` so `C-h i d m Jotain RET` works out of the box), `jylhisEmacsPackages` (the same package/config layer on the fork, currently experimental), `jotainEmacsNoGui` / `jotainEmacsPackagesNoGui` (the `noGui` terminal build and its full distribution, used by the nix-on-droid module), and `sonarlintLs` (the SonarLint language server from nixpkgs, for `M-x jotain-sonarlint`).
