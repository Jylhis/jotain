...
```bash
nix-build default.nix                                  # Emacs 30.2 mainline (binary cached)
nix-build default.nix --arg noGui true                 # terminal-only
nix-build default.nix --arg withPgtk true              # pure GTK (Wayland)
nix-build default.nix --arg variant '"git"'             # bleeding-edge master
nix-build default.nix --arg variant '"igc"'             # incremental GC branch
nix-build default.nix --arg variant '"macport"'         # macOS macport fork
```
...
- **`default.nix`** — Comprehensive Nix expression to build GNU Emacs from source. Supports multiple source variants (mainline, git, unstable, macport, igc), GUI toolkits (GTK3, pgtk, NS, X11, no-GUI), native compilation, tree-sitter, and macOS-specific patches. Wraps `nixpkgs` `emacs30` with `override`/`overrideAttrs`.
